...
mkdir -p ~/fossa && curl -L https://github.com/fossas/fossa-installer/archive/v0.0.20.tar.gz | tar -zxv -C ~/fossa --strip-components=1 && chmod a+x ~/fossa/boot.sh && sudo ln -sf ~/fossa/boot.sh /usr/local/bin/fossa && cd ~/fossa
