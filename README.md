...
mkdir -p ~/fossa && curl -L https://github.com/fossas/fossa-installer/archive/v0.0.7.tar.gz | tar -zxv -C ~/fossa --strip-components=1 && chmod a+x ~/fossa/boot.sh && ln -sf ~/fossa/boot.sh /usr/local/bin/fossa && fossa init
